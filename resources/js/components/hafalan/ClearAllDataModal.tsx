            ...
            <div className="bg-card text-card-foreground border-border w-full max-w-md rounded-2xl border p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center justify-between border-b border-border pb-3">
                    <div className="flex items-center gap-2.5">
                        <div className="flex size-10 items-center justify-center rounded-xl bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-500/30">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-rose-500 font-extrabold">ZONA BAHAYA: Tindakan Permanen!</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="bg-rose-500/10 border border-rose-500/30 rounded-xl p-4 space-y-2 text-xs">
                    <div className="font-bold text-foreground text-sm">
                        Anda akan mereset dan mengosongkan <span className="text-rose-600 dark:text-rose-400 font-extrabold">SELURUH APLIKASI</span>:
                    </div>
                    <ul className="list-disc list-inside space-y-1 text-muted-foreground text-xs font-medium">
                        <li>Semua data murid dari seluruh 12 kelas ({totalStudentCount} siswa)</li>
                        <li>Seluruh riwayat & centang hafalan Al-Qur'an dari seluruh murid</li>
                        <li>Seluruh log aktivitas riwayat</li>
                    </ul>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="space-y-1.5">
                        <Label className="text-xs font-bold text-foreground">
                            Ketik kata kunci di bawah untuk konfirmasi reset total:
                        </Label>
                        <div className="bg-muted/60 p-2.5 rounded-lg text-xs font-mono font-bold text-rose-600 dark:text-rose-400 select-all border border-rose-500/30 text-center tracking-wider">
                            {expectedText}
                        </div>
                        <Input
                            type="text"
                    ...
                    <div className="flex gap-2 pt-2">
                        ...
                        >
                            <Trash2 className="size-4 mr-1.5" /> Ya, Reset Seluruh Data
                        </Button>
                    </div>
                </form>
            </div>
